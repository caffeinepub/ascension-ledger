...
export function isDraftEnvironment(): boolean {
  ...
  return (
    window.location.hostname === "draft.caffeine.xyz" ||
...
export async function unregisterServiceWorkers(): Promise<void> {
  if ("serviceWorker" in navigator) {
    try {
      ...
      console.log(
        "[Cache Control] Unregistering service workers:",
      ...
      console.error(
        "[Cache Control] Failed to unregister service workers:",
        error,
      );
      throw error;
    }
...
export async function initializeDraftEnvironment(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[Cache Control] Failed to initialize draft environment:",
      error,
    );
  }
}
