...
export function SkillsPage() {
  ...
  const getDisciplineSkillTier = (
    requiredLevel: number,
  ): { label: string; color: string } => {
    if (requiredLevel <= 3) return { label: "Novice", color: "text-gray-400" };
    if (requiredLevel <= 6)
      return { label: "Apprentice", color: "text-green-400" };
                      ...
                      {skill.requirements.minStats.length > 0 && (
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-white/80">
                            {COPY.skills.requiresStats}
                          </p>
                          {skill.requirements.minStats.map(
                            ([statIndex, minValue]) => {
                              const index = Number(statIndex);
                              const statNames = [
                                "Academics",
                  ...
                  <CardFooter>
                    {unlocked ? (
                      ...
                      <Button
                        ...
                        className="w-full"
                      >
                        {unlockSkill.isPending
                          ? COPY.skills.unlocking
                          : COPY.skills.unlock}
                      </Button>
                    )}
                  </CardFooter>
        ...
        <>
          ...
          <section>
            ...
            <p className="mb-6 text-sm text-white/60">
              Unlock powerful discipline abilities as you level up. Higher-tier
              skills require greater mastery.
            </p>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
