...
export function CheatStorePage() {
  ...
  const { data: purchasesToday, isLoading: purchasesLoading } =
    useGetUserCheatPurchasesToday();
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <span className="font-semibold text-accent">How it works:</span> Earn
        credits by completing missions and tasks. Spend them here on real-life
        rewards. Daily limits reset at midnight UTC.
      </div>
      ...
      {!isLoading && cheatItems && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {cheatItems.map((item) => {
            ...
            const isDisabled =
              isSoldOut || cannotAfford || purchaseCheat.isPending;
            const isPurchasingThis =
              purchaseCheat.isPending && purchaseCheat.variables === item.id;
              ...
              >
                ...
                {isSoldOut && (
                  <div className="absolute top-3 right-3">
                    <Badge
                      variant="secondary"
                      className="text-xs bg-white/10 text-white/50 border-white/10"
                    >
                ...
                {/* Item Name */}
                <h3
                  className={`font-semibold text-base leading-tight pr-8 ${isDisabled ? "text-white/50" : "text-white"}`}
                >
                  {item.name}
                </h3>
                ...
                <div className="flex flex-col gap-2 pt-1 border-t border-white/5">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        <Coins
                          className={`h-4 w-4 ${isDisabled ? "text-white/30" : "text-primary"}`}
                        />
                        <span
                          className={`text-sm font-bold ${isDisabled ? "text-white/40" : "text-primary"}`}
                        >
                          {creditCost} credits
                        </span>
                      </div>
                  ...
                  {!isSoldOut && cannotAfford && (
                    <div
                      className="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 border border-destructive/30"
                    ...
                    >
                      ...
                      <span className="text-xs text-destructive/80 font-medium">
                        Insufficient credits — need {creditCost - userCredits}{" "}
                        more
                      </span>
                    </div>
                  )}
