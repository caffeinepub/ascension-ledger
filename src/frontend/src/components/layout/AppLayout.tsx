...
import { Swords, TrendingUp, Zap, User, Activity } from 'lucide-react';
...
export function AppLayout({ children, currentScreen, onNavigate }: AppLayoutProps) {
  ...
  return (
    <div className="flex h-screen flex-col bg-transparent dark">
      {/* Header */}
      <header className="border-b border-border" style={{ background: 'rgba(0, 0, 0, 0.8)', backdropFilter: 'blur(10px)' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar Navigation */}
        <nav className="w-64 border-r border-border p-4" style={{ background: 'rgba(0, 0, 0, 0.8)', backdropFilter: 'blur(10px)' }}>
              ...
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-md px-4 py-3 text-left transition-colors ${
                  currentScreen === item.id
                    ...
                    : 'text-white/90 hover:bg-accent hover:text-accent-foreground'
                }`}
              >
                {item.icon}
      ...
      {/* Footer */}
      <footer className="border-t border-border py-4" style={{ background: 'rgba(0, 0, 0, 0.8)', backdropFilter: 'blur(10px)' }}>
