...
export function ProfileSetupDialog({ open }: ProfileSetupDialogProps) {
  ...
  return (
    <Dialog open={open}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-xl sm:text-2xl text-white/95">
            {COPY.profileSetup.welcomeTitle}
          </DialogTitle>
          ...
          <DialogFooter>
            ...
            >
              {initializeProfile.isPending
                ? COPY.profileSetup.creatingProfile
                : COPY.profileSetup.beginButton}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
