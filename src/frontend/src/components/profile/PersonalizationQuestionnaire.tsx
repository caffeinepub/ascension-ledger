...
export function PersonalizationQuestionnaire({
  ...
  return (
    <Dialog open={open}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-2xl text-white/95">
            {COPY.personalization.title}
          </DialogTitle>
        ...
        <div className="py-4 sm:py-6 space-y-4 sm:space-y-6">
          ...
          <div className="flex items-center justify-between text-xs sm:text-sm text-white/60">
            <span>
              {COPY.personalization.questionLabel} {currentQuestion + 1}{" "}
              {COPY.personalization.of} {questions.length}
            </span>
            <span>
          ...
          <div className="space-y-4">
            <h3 className="text-base sm:text-lg font-medium text-white/95 leading-snug">
            ...
            >
              <div className="space-y-2 sm:space-y-3">
                {questions[currentQuestion].options.map((option, index) => (
                  <div
                    key={option}
                    className="flex items-center space-x-3 p-3 sm:p-4 rounded-lg border border-white/10 hover:border-primary/50 transition-colors bg-black/20 min-h-[44px]"
                  >
                    <RadioGroupItem
                      value={option}
                      id={`option-${index}`}
                      className="border-white/40 min-w-[20px] min-h-[20px]"
                    />
                    <Label
            ...
            {!isLastQuestion ? (
              ...
              >
                {submitAnswers.isPending
                  ? COPY.personalization.submitting
                  : COPY.personalization.submit}
              </Button>
            )}
          </div>
        </DialogFooter>
